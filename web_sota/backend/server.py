    ...
    def query(self, limit=50, offset=0, level=None, kind=None, search=None, sort="desc", after_id=None):
        ...
        if after_id:
            try:
                ...
                logger.debug("Invalid after_id format", after_id=after_id)
        if level:
...
app = FastAPI(title="Unity3D MCP", lifespan=lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
@app.get("/api/health")
async def health():
    return {"status": "ok", "server": "Unity3D MCP", "version": "0.1.0"}
