...
class TestVRChatAuthentication:
    ...
    async def test_authenticate_success(self, vrchat_manager, mock_vrchat_auth_response):
        ...
        with patch("aiohttp.ClientSession") as mock_session:
            ...
            session_instance.get = MagicMock(return_value=MagicMock(
...
class TestAvatarValidation:
    ...
    @pytest.mark.asyncio
    async def test_validate_avatar_sdk_not_installed(
        self, vrchat_manager, mock_unity_project
    ):
        """Test validation fails when SDK not installed."""
        result = await vrchat_manager.validate_avatar(
            "Assets/Prefabs/Avatar.prefab",
            str(mock_unity_project)
        )
    ...
    @pytest.mark.asyncio
    async def test_validate_avatar_prefab_not_found(
        self, vrchat_manager, mock_vrchat_project
    ):
        """Test validation with missing prefab."""
        result = await vrchat_manager.validate_avatar(
            "Assets/Prefabs/NonExistent.prefab",
    ...
    @pytest.mark.asyncio
    async def test_validate_avatar_success(
        self, vrchat_manager, mock_vrchat_project, mock_avatar_prefab
    ):
...
class TestAvatarUpload:
    ...
    @pytest.mark.asyncio
    async def test_upload_sdk_not_installed(
        self, vrchat_manager, mock_unity_project
    ):
    ...
    @pytest.mark.asyncio
    async def test_upload_not_authenticated(
        self, vrchat_manager, mock_vrchat_project, mock_avatar_prefab, clean_env
    ):
...
class TestPerformanceRanks:
    ...
    @pytest.mark.parametrize("polygon_count,expected_valid", [
        (10000, True),
        (32000, True),
        (70000, True),
        (70001, False),
    ...
    @pytest.mark.parametrize("material_count,expected_rank", [
        (1, "Excellent"),
        (4, "Good"),
        (8, "Medium"),
        (16, "Poor"),
